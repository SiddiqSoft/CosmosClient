...
# Friendly name overrides for known repositories/packages across SiddiqSoft projects
DISPLAY_NAMES = {
...
def parse_cmake_file(filepath):
    ...
    for line in lines:
        ...
        if "MATCHES [Mm][Ss][Vv][Cc]" in stripped or 'STREQUAL "MSVC"' in stripped:
            ...
            if "MATCHES" not in stripped and "STREQUAL" not in stripped:
                current_scope = "Core"
...
def detect_project_name(root_path):
    """
    Tries to detect project name from CMakeLists.txt or directory name.
    """
    cm_file = Path(root_path) / "CMakeLists.txt"
    if cm_file.exists():
        with open(cm_file, "r", encoding="utf-8") as f:
            match = re.search(r'project\s*\(\s*([A-Za-z0-9_\-]+)', f.read(), re.IGNORECASE)
            if match:
                return match.group(1)
    return Path(root_path).name


def generate_markdown(dependencies, project_name="cosmoscl"):
    """
    ...
    core_deps = [
        d for d in dependencies if d["scope"] == "Core"
    ]
    test_deps = [
        d for d in dependencies if d["scope"] == "Test"
    ]
